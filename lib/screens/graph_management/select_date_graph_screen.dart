...
class SelectDateScreen extends StatefulWidget {
  ...
  const SelectDateScreen({Key? key, required this.child, required this.testList, required this.isDate}) : super(key: key);
...
class _SelectDateScreenState extends State<SelectDateScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
        ...
        child: Scaffold(
          appBar: SearchAppBar(
              context, (widget.child.name + "의 테스트 날짜(or 제목) 선택")),
  ...
  Widget dataTile(Test test, int index) {
    return buildListTile(
      titleText: test.date.toString(),
      subtitleText: "평균성공률: ${test.average}%",
      onTap: () {
        Navigator.push(context, MaterialPageRoute(builder: (context) => RealGraph(
          test: test,
          isDate: true,
