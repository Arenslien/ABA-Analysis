...
class FireStoreService {
  ...
  CollectionReference _childResult = FirebaseFirestore.instance.collection('Child Result');
  ...
  Future createUser(ABAUser abaUser) async {
    ...
    return await _user.doc(abaUser.email).set(abaUser.toMap()).then((value) => print('[사용자: ${abaUser.email}}] - 추가 완료')).catchError((error) => print('[사용자: ${abaUser.email}}] - 추가 실패\n에러 내용: $error'));
  }

  // 사용자 읽기
  Future<ABAUser?> readUser(String email) async {
    // 해당 이메일에 대한 User 정보 가져오기
    dynamic data = await _user.doc(email).get().then((DocumentSnapshot snapshot) => snapshot.data());
  ...
  Future<List<ABAUser>> readUnapprovedUser() async {
    ...
    // 모든 아이들 데이터 가져오기
    await _user.where('approval-status', isEqualTo: false).get().then((QuerySnapshot snapshot) => snapshot.docs.forEach((document) {
  ...
  Future<List<ABAUser>> readApprovedUser() async {
    ...
    // 모든 아이들 데이터 가져오기
    await _user.where('approval-status', isEqualTo: true).get().then((QuerySnapshot snapshot) => snapshot.docs.forEach((document) {
          dynamic data = document.data();
          ABAUser abaUser = ABAUser(
            email: data['email'],
            password: data['password'],
  ...
  // 사용자 수정
  Future updateUser(String email, String nickname, String duty, bool approvalStatus, bool deleteRequest) async {
    return _user
        .doc(email)
        .update({
          'nickname': nickname,
  ...
  Future deleteUser(String email) async {
    return await _user.doc(email).delete().then((value) => print('[사용자: $email] - 삭제 완료')).catchError((error) => print('[사용자: $email] - 삭제 실패\n에러 내용: $error'));
  }

  Future<bool> checkUserWithEmail(String email) async {
    try {
      QueryDocumentSnapshot abaUser = await _user.where('email', isEqualTo: email).get().then((snapshot) => snapshot.docs[0]);
  ...
  Future createChild(Child child) {
    // 데이터베이스에 Child 문서 추가
    return _child.doc(child.childId.toString()).set(child.toMap()).then((value) => print('[아동: ${child.name}] - 추가 완료')).catchError((error) => print('[아동: ${child.name}] - 추가 실패\n에러 내용: $error'));
  }
  ...
  Future<List<Child>> readAllChild(String email) async {
    ...
    // 모든 아이들 데이터 가져오기
    await _child.where('teacher-email', isEqualTo: email).get().then((QuerySnapshot snapshot) => snapshot.docs.forEach((document) {
          dynamic data = document.data();
          Timestamp date = data['birthday'];
          Child child = Child(childId: data['child-id'], teacherEmail: data['teacher-email'], name: data['name'], birthday: date.toDate(), gender: data['gender']);
          children.add(child);
        }));
  ...
  Future<Child?> readChild(int childId) async {
    ...
    dynamic data = await _child.doc(childId.toString()).get().then((DocumentSnapshot snapshot) => snapshot.data());
    if (data == null) return null;
  ...
  Future deleteChild(int childId) async {
    await _child.doc(childId.toString()).delete().then((value) => print('[아동: $childId] - 추가 완료')).catchError((error) => print('[아동: $childId] - 추가 실패\n에러 내용: $error'));
  }
  ...
  Future addProgramField(String title) async {
    ...
    _programField.doc(id.toString()).set(programField.toMap()).then((value) => print("프로그램 영역이 추가 되었습니다.")).catchError((error) => print("프로그램 영역 추가를 실패했습니다. : $error"));
  }

  Future deleteProgramField(int index) async {
    await _programField.doc(index.toString()).delete().then((value) => print('프로그램 영역 삭제 완료')).catchError((error) => print('프로그램 영역 삭제 실패\n에러 내용: $error'));
  }
  ...
  Future addSubField(SubField subField) async {
    // 새로운 Sub Field 추가
    await _subField.doc(subField.id.toString()).set(subField.toMap()).then((value) => print("하위 영역이 추가 되었습니다.")).catchError((error) => print("하위 영역 추가를 실패했습니다. : $error"));
  }

  Future deleteSubField(int index) async {
    // SubField 삭제
    await _subItem.doc(index.toString()).delete().then((value) => print('하위 영역 삭제 완료')).catchError((error) => print('하위 영역 삭제 실패\n에러 내용: $error'));
  }
  ...
  Future addSubItem(SubItem subItem) async {
    // 새로운 Sub Item 추가
    await _subItem.doc(subItem.id.toString()).set(subItem.toMap()).then((value) => print("하위 목록이 추가 되었습니다.")).catchError((error) => print("하위 목록 추가를 실패했습니다. : $error"));
  }

  Future deleteSubItem(int index) async {
    // SubItem 삭제
    await _subItem.doc(index.toString()).delete().then((value) => print('하위 목록 삭제 완료')).catchError((error) => print('하위 목록 삭제 실패\n에러 내용: $error'));
  }
  ...
  Future<List<SubItem>> readAllSubItem() async {
    ...
    List<QueryDocumentSnapshot> docs = await _subItem.get().then((QuerySnapshot snapshot) => snapshot.docs);
  ...
  Future createTest(Test test) async {
    // 데이터베이스에 Test 문서 추가
    return await _test.doc(test.testId.toString()).set(test.toMap()).then((value) => print(' 테스트가 성공적으로 추가되었습니다.')).catchError((error) => print('테스트를 추가하지 못했습니다.\n에러 내용: $error'));
  }

  // Test 복사
  Future<Test> copyTest(Test test) async {
    Test copiedTest = Test(testId: await updateId(AutoID.test), childId: test.childId, date: DateTime.now(), title: '${test.title} 복사본', isInput: false);
    await _test.doc(copiedTest.testId.toString()).set(copiedTest.toMap()).then((value) => print(' 테스트가 성공적으로 복사되었습니다.')).catchError((error) => print('테스트를 복사하지 못했습니다.\n에러 내용: $error'));
    return copiedTest;
  }
  ...
  Future<Test?> readTest(int testId) async {
    ...
    dynamic data = await _test.doc(testId.toString()).get().then((DocumentSnapshot snapshot) => snapshot.data());
    if (data == null) return null;

    // Document data 기반 Test 객체 생성
    Test test = Test(testId: data['test-id'], childId: data['child-id'], date: data['date'], title: data['title'], isInput: data['is-input']);
  ...
  Future<List<Test>> readAllTest() async {
    ...
    List<QueryDocumentSnapshot> docs = await _test.get().then((QuerySnapshot snapshot) => snapshot.docs);
    ...
    for (QueryDocumentSnapshot doc in docs) {
      ...
      // 테스트 생성 & 추가
      Test test = Test(testId: data['test-id'], childId: data['child-id'], date: date.toDate(), title: data['title'], isInput: data['is-input']);
  ...
  Future<List<Test>> readTestList(int childId) async {
    List<Test> testList = [];
    // 해당 childId에 대한
    List<QueryDocumentSnapshot> docs = await _test.where('child-id', isEqualTo: childId).get().then((QuerySnapshot snapshot) => snapshot.docs);
  ...
  Future deleteTest(int testId) async {
    await _test.doc(testId.toString()).delete().then((value) => print("테스트가 삭제되었습니다.")).catchError((error) => "테스트를 삭제하지 못했습니다. : $error");
  }
  ...
  Future createTestItem(TestItem testItem) async {
    // 데이터베이스에 Test 문서 추가
    return await _testItem.doc(testItem.testItemId.toString()).set(testItem.toMap()).then((value) => print('테스트가 성공적으로 추가되었습니다.')).catchError((error) => print('테스트를 추가하지 못했습니다.\n에러 내용: $error'));
  }
  ...
  Future<TestItem> copyTestItem(int testId, int childId, TestItem testItem) async {
    ...
    await _testItem.doc(copiedTestItem.testItemId.toString()).set(copiedTestItem.toMap()).then((value) => print(' 테스트아이템이 성공적으로 복사되었습니다.')).catchError((error) => print('테스트아이템을 복사하지 못했습니다.\n에러 내용: $error'));
  ...
  Future<TestItem?> readTestItem(int index) async {
    ...
    dynamic data = await _testItem.doc(index.toString()).get().then((DocumentSnapshot snapshot) => snapshot.data());
    if (data == null) return null;
  ...
  Future readAllTestItem() async {
    ...
    List<QueryDocumentSnapshot> docs = await _testItem.get().then((QuerySnapshot snapshot) => snapshot.docs);
  ...
  Future deleteTestItem(int testItemId) async {
    await _testItem.doc(testItemId.toString()).delete().then((value) => print("테스트 아이템이 삭제되었습니다.")).catchError((error) => "테스트 아이템을 삭제하지 못했습니다. : $error");
  }
  ...
  Future<dynamic> readAutoIDDocumentData() async {
    ...
    return _autoId.doc('AutoID').get().then((DocumentSnapshot snapshot) => snapshot.data());
  }
